...
ClapTrap::ClapTrap(std::string name) : m_name(name)
{
    ...
    std::cout << WHITE "ClapTrap " << ITAL << m_name << REST << " constructor" << called REST;
}
...
{
    std::cout << "ClapTrap " << ITAL << m_name << REST
...
{
    std::cout << "ClapTrap " << ITAL << m_name << REST 
                << GREEN " got repaired and gained " << n << " health \n" REST;
}

void    ClapTrap::printDamage(unsigned int n)
{
    std::cout << YELL "causing " << n << " points of damage! \n" REST;
}
